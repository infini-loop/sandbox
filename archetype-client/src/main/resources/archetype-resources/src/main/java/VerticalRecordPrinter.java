...
import static ${package}.AlignedTablePrinter.consoleWidth;
import static ${package}.AlignedTablePrinter.formatValue;
import static ${package}.AlignedTablePrinter.maxLineLength;
...
{
    private static final Splitter LINE_SPLITTER = Splitter.on('${symbol_escape}n');
    ...
    {
        if (rowCount == 0) {
            writer.append("(no rows)${symbol_escape}n");
        }
        writer.flush();
    }
            ...
            throws IOException
    {
        ...
        for (List<?> row : rows) {
            ...
            writer.append(header).append('${symbol_escape}n');
