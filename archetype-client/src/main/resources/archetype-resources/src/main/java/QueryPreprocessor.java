...
import static ${package}.ConsolePrinter.REAL_TERMINAL;
...
public final class QueryPreprocessor
{
            ...
            throws QueryPreprocessorException
    {
        ...
        try {
            ...
            if (REAL_TERMINAL) {
                System.out.print("${symbol_escape}r" + Strings.repeat(" ", PREPROCESSING_QUERY_MESSAGE.length()) + "${symbol_escape}r");
                System.out.flush();
            }
        ...
        Future<String> task = executeInNewThread("Query preprocessor", () -> {
            ...
            if (exitCode != 0) {
                ...
                throw new QueryPreprocessorException("Query preprocessor exited " + exitCode +
                        errorMessage.map(message1 -> "${symbol_escape}n===${symbol_escape}n" + message1 + "${symbol_escape}n===").orElse(""));
            }
            return result;
        });
